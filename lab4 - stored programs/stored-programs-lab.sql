...
# create view 
create view names_dept as select firstName, lastName, department from details;

# select all from created view 
select * from names_dept;

select * from names_dept where firstName like "Joe";
UPDATE names_dept SET department = "Researcher" WHERE firstName LIKE "Joe" AND lastName LIKE "Mullins"

# create procedure called spUpdateDetailsRate
delimiter //
...
update details set rate="15" WHERE personID="id";
...
# create procedure called spUpdateDetailsRate
delimiter //
create procedure spUpdateDetailsRate_withOutput (
personID INT,
newRate INT,
...
# check if this needs to be declared again or not
DELIMITER //
...
SELECT SUM(rate*hours) INTO wages FROM details WHERE id=PersonID;
RETURN wages;
END//

SELECT fnCalculateWages(3);
